  ...
  reader.onload = (e) => {
    try {
      ...
      rows.slice(1).forEach((row) => {
        if (row.length >= 5) {
          const studentId = String(row[1]).padStart(5, "0"); // เลขประจำตัว
          const prefix = row[3] || ""; // คำนำหน้า
          const firstName = row[4] || ""; // ชื่อ
          const lastName = row[5] || ""; // สกุล
      ...
      showToast(`นำเข้าข้อมูล ${Object.keys(students).length} นักเรียน`);
    } catch (error) {
...
studentIdInput.addEventListener("blur", () => {
  ...
  if (students[studentId]) {
    studentNameInput.value = students[studentId].fullName;
  }
});
